-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/Sambaran/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/Sambaran/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/Sambaran/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/Sambaran/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/Sambaran/outputs' USING PigStorage('\t');